...
CREATE TABLE IF NOT EXISTS `exam_announcement` (
    ...
    `exam_id` int DEFAULT NULL COMMENT '关联考试ID',
...
-- 2. exam_record 确保 exam_status 存在（如已存在可跳过此句）
...
-- 3. exam 新增自动公示/录屏字段
ALTER TABLE `exam`
    ADD COLUMN `auto_publish` tinyint(1) DEFAULT 0 COMMENT '是否自动公示成绩' AFTER `exam_type`,
...
CREATE TABLE IF NOT EXISTS `exam_recording` (
    ...
    `record_id` int DEFAULT NULL COMMENT '考试记录ID',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='考试录屏记录';
